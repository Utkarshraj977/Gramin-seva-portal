...
const DoctorUserDashboard = () => {
    ...
    // 2. NEW STATE: Store Current Patient Info & Active Chat
    const [currentUser, setCurrentUser] = useState(null);
    const [activeChatDoctor, setActiveChatDoctor] = useState(null);

    // 3. Fetch Data (Doctors + Current Patient)
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch All Doctors
                const docRes = await axios.get('http://localhost:8000/api/v1/doctor/getalldoctor', {
                    withCredentials: true
                });
                setDoctors(docRes.data.data);
                setFilteredDoctors(docRes.data.data);

                // Fetch Current Logged-in Patient (Required for Chat ID)
                // Assuming route exists based on your pattern: /api/v1/patient/currentpatient 
                // If this route is different, please update it.
                const userRes = await axios.get('http://localhost:8000/api/v1/patient/currentpatient', {
                    withCredentials: true
                });
    ...
    const handleConnect = async (doctorId) => {
        ...
        try {
            ...
            alert("Success! You are now connected with this doctor.");
        } catch (error) {
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="relative w-full max-w-4xl h-[85vh] flex flex-col">
                         ...
                         <div className="flex-1 bg-white rounded-2xl overflow-hidden shadow-2xl border border-slate-200">
                             <ChatRoom 
                                // 1. ROOM ID: [DoctorID, PatientID].sort() ensures same ID for both
