  ...
  const handleActionSubmit = async () => {
    ...
    try {
      if (actionType === "resolve") {
          await complaintAdmin.resolve_complaint(selectedComplaint._id, responseText); 
      } else {
          await complaintAdmin.reject_complaint(selectedComplaint._id, responseText);
      }
      ...
      <aside className="w-72 bg-slate-900 border-r border-slate-800 flex-col hidden md:flex z-20">
        <div className="p-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-white text-lg">OFFICIAL</h1>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="bg-slate-950/50 rounded-xl p-4 mb-3 border border-slate-800">
            <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Jurisdiction</p>
            ...
            {activeTab === "overview" && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard title="Total" value={stats.total} icon={<Briefcase/>} color="text-blue-400" bg="bg-blue-500/10" border="border-blue-500/20" />
                  <StatCard title="Pending" value={stats.pending} icon={<Clock/>} color="text-amber-400" bg="bg-amber-500/10" border="border-amber-500/20" />
                  <StatCard title="Resolved" value={stats.resolved} icon={<CheckCircle2/>} color="text-emerald-400" bg="bg-emerald-500/10" border="border-emerald-500/20" />
                  <StatCard title="Requests" value={pendingReqs.length} icon={<UserPlus/>} color="text-purple-400" bg="bg-purple-500/10" border="border-purple-500/20" />
                </div>
                ...
                >
                    ...
                    <div>
                        ...
                        {acceptedReqs.length > 0 ? (
                            ...
                            <p className="text-slate-500 text-sm italic">No connections yet</p>
                        )}
                    </div>
                </motion.div>
            )}
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-6 mb-8 pb-8 border-b border-slate-800 relative z-10">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 mt-1">Update official details</p>
                    </div>
                  </div>

                  <form onSubmit={handleProfileUpdate} className="space-y-6 relative z-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <InputGroup label="Designation" value={profileData.designation} onChange={(v) => setProfileData({...profileData, designation: v})} placeholder="Gram Pradhan"/>
                      <InputGroup label="Office Location" value={profileData.location} onChange={(v) => setProfileData({...profileData, location: v})} placeholder="Panchayat Bhawan"/>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-800">
                        <p className="text-xs text-slate-400 uppercase font-bold mb-4 flex items-center gap-2"><Clock size={14}/> Office Hours</p>
                    ...
                    <div className="pt-4 flex justify-end">
                        <button type="submit" className="bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg">
                          <RefreshCcw size={18} /> Save
                        </button>
                    </div>
                  </form>
                </div>
            ...
            <div className="p-6 space-y-6">
               ...
               <div>
                 <label className="text-xs text-slate-400 uppercase font-bold mb-2 block pl-1">
                   {actionType === "resolve" ? "Action Taken / Response" : "Reason for Rejection"}
                 </label>
                 <textarea 
                   rows="5"
                   className="w-full bg-slate-800 border border-slate-700 rounded-xl p-4 text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-none text-sm"
                   placeholder={actionType === "resolve" ? "Describe how the issue was fixed..." : "Explain why rejected..."}
                   value={responseText}
...
const NavButton = ({ active, onClick, icon: Icon, label, badge }) => (
  <button onClick={onClick} className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-all font-medium text-sm border ${active ? "bg-red-600 border-red-500 text-white shadow-lg" : "bg-transparent border-transparent text-slate-400 hover:bg-slate-800 hover:text-white"}`}>
...
const StatCard = ({ title, value, icon, color, bg, border }) => (
  <div className={`p-5 rounded-2xl border ${bg} ${border} flex items-center justify-between relative overflow-hidden`}>
     <div className="relative z-10">
       <p className="text-slate-400 text-xs uppercase font-bold mb-1">{title}</p>
       <h3 className={`text-3xl font-bold ${color}`}>{value || 0}</h3>
     </div>
     <div className={`w-12 h-12 rounded-xl bg-slate-900 flex items-center justify-center ${color} shadow-sm relative z-10`}>{icon}</div>
  </div>
);
...
const InputGroup = ({ label, type = "text", value, onChange, placeholder }) => (
  <div className="space-y-2">
     ...
     <input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl px-4 py-3 outline-none focus:border-red-500 transition-colors"/>
  </div>
);
...
const RequestCard = ({ user, type, onAccept, onReject }) => (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col items-center text-center shadow-lg hover:border-slate-600 transition-all relative overflow-hidden">
        ...
        <div className="w-20 h-20 rounded-full border-4 border-slate-800 bg-slate-950 flex items-center justify-center mb-4 overflow-hidden relative">
            {user?.avatar?.url ? <img src={user.avatar.url} alt="User" className="w-full h-full object-cover"/> : <User size={40} className="text-slate-600"/>}
        ...
        <h3 className="text-lg font-bold text-white mb-1">{user?.fullName || "User"}</h3>
        ...
        {type === "pending" ? (
            ...
            <div className="w-full mt-auto">
                ...
                <button onClick={onReject} className="w-full py-2 rounded-xl bg-slate-800 text-red-400 border border-slate-700 font-bold text-xs hover:bg-red-900/20 hover:border-red-500/30 transition-all flex items-center justify-center gap-2"><Trash2 size={14}/> Remove</button>
            </div>
        )}
    </div>
);
...
const ComplaintCard = ({ data, onResolve, onReject }) => (
  <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden hover:border-slate-600 transition-all flex flex-col h-full hover:shadow-2xl">
     <div className="p-5 border-b border-slate-800 bg-slate-800/30 flex justify-between items-start">
       <div className="flex gap-4">
           ...
           <div>
             <h4 className="text-white font-bold text-base">{data.title || "Untitled"}</h4>
             <p className="text-xs text-slate-400 mt-1 flex items-center gap-1.5">
                <User size={12}/> {data.userInfo?.fullName || "Citizen"}
                <span className="text-slate-600">•</span>
                <Clock size={12}/> {data.createdAt ? new Date(data.createdAt).toLocaleDateString() : "N/A"}
             </p>
           </div>
       </div>
     ...
     <div className="p-5 flex-1 flex flex-col">
       <div className="flex flex-wrap gap-2 mb-4">
           ...
           <span className="px-2.5 py-1 rounded-md bg-slate-950 text-slate-400 text-[10px] font-bold uppercase border border-slate-800 flex items-center gap-1"><MapPin size={10}/> {data.location || "N/A"}</span>
       </div>
       <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 mb-4">
           <p className="text-slate-300 text-sm">"{data.message}"</p>
       </div>
       {data.complaintImage?.url && (
           <div className="h-40 w-full rounded-xl bg-black overflow-hidden border border-slate-800 mb-4">
             <img src={data.complaintImage.url} alt="Proof" className="w-full h-full object-cover" />
           </div>
       )}
       {data.adminResponse && (
           <div className={`mt-auto p-4 rounded-xl border text-xs ${data.status === "Resolved" ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-400" : "bg-red-500/5 border-red-500/20 text-red-400"}`}>
              <strong className="block mb-1.5 uppercase opacity-80">Response:</strong>
              {data.adminResponse}
           </div>
       )}
     </div>
     {data.status && data.status.toLowerCase() === "pending" && (
        <div className="p-4 border-t border-slate-800 bg-slate-950/50 grid grid-cols-2 gap-3">
           <button onClick={onResolve} className="py-2.5 rounded-xl bg-emerald-600/10 text-emerald-500 hover:bg-emerald-600 hover:text-white border border-emerald-600/20 font-bold text-xs flex items-center justify-center gap-2 transition-all"><CheckCircle2 size={16}/> Resolve</button>
...
const StatusBadge = ({ status }) => {
  const normalized = status ? status.charAt(0).toUpperCase() + status.slice(1).toLowerCase() : "Unknown";
  const config = { 
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase border ${config[normalized] || config.Pending} flex items-center gap-1.5`}>
       <span className={`w-1.5 h-1.5 rounded-full ${normalized === 'Pending' ? 'animate-pulse bg-current' : 'bg-current'}`}></span> {normalized}
    </span>
  );
};
